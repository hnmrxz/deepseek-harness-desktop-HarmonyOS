...
**点一下时间线格子，就能读到完整的系统提示词（P8-5）**。这一轮做的是 `docs/09-安全与API错误呈现审计.md` 的 §2.2 与 §4，事实源是**本仓自己的审计 + 代码路径**（不是官方 Web：上游没有"系统提示词该不该给用户看"这条契约，但它的 `system/message` 类型与我们的 `internal` 标记是同一件事）。查下去的第一个洞就在眼前：**时间线是可点的，而内部事件也会产生格子**（`system/message` 是 kind=MESSAGE ⇒ 有格子），点开就是「事件详情」，而那份详情此前把 `body` 原样画出来 ⇒ **点一下就拿到整段系统提示词**（未识别事件则把整段原始载荷画出来）。这与审计「轨迹详情默认不得展示 `internal=true` 的正文」直接冲突。现在：正文类字段（正文/思考/工具参数/工具结果/错误原文）在内部条目上**一律不画**，只留**结构化事实**（类别、状态、工具名、模型名、字符数、耗时），并补一行说明——"不显示"要是一个**解释**（含字符数、且写清是安全口径而非数据丢失），而不是一片空白。**判据只有一处**：`Turns.mayExposeBodyToUser()`（来源事实就是上游给的 `internal`，不靠正文特征猜），出口取值只有一处：`userFacingBodyOf()`——复制（`TurnView`/`ConversationPane`）与引用（`Index.quoteIntoDraft`）都改从它取，内部条目自然落进"没有可复制的正文"分支。第二个洞在**错误文案**：Host 原文是排查的第一手证据，我们把它原样附在文案后面（`（Host 原文：…）`），而它常含 `?token=…` 这类凭据，且这条文本会进**三个**用户可达出口（会话错误行、诊断报告、剪贴板）。现在三处拼接收成一个 `hostDetail()`，一律先过脱敏；脱敏规则本身从 `Notify` 搬进**零依赖** `model/Redact.ets`（`looksSensitive` / `scrubCredentials` / `collapseWhitespace`）——搬家的理由很直接：规则留在通知里，就等于"只有通知这条出口脱敏"，而搬成零依赖之后 fixture 能**直接执行**它，五种凭据形状各有一条断言钉住。顺带核实并写进文档的两件事（**不是缺陷**）：通知早就有 `assertNoSensitiveContent` + `sanitizeDetail` 两道闸；旁路日志逐行看过，只有 id 前缀、条数、错误码，没有正文。**这一轮还修好了门禁自己**：死代码门禁的剥壳函数在剥块注释时**不保留换行**，于是"跳过 import 块自身"的行号与原文件行号错位 ⇒ 零使用 import 这条规则**长期假阴性**（它的计数被自己的 import 行喂饱了）。对齐行号后，仓库里 **44 个真正没人用的 import** 立刻现形（`MainShell`/`DetailPane`/`Fs`/`formatDuration`/`Redact` 搬走后的 `REDACTED`…），全部删净；计数口径也改成"剥注释、**保留字符串**"（模板串 `${SURFACE.apiChannel}` 里的用法算使用，否则又会误报）。fixture **922 → 947 条**（新增 25 条：5 种凭据形状 + 脱敏后仍保留证据 + 四种内部条目的详情 + 出口判据），归真验证：把详情改回"内部正文照画"且不脱敏 ⇒ **9 条当场红**；接线门禁 32 → **33 项**（「内部内容隔离」钉住"判据/取值/脱敏三处都必须有调用点"）。**仍缺（已登记）**：`session/search` 的**内容命中来自 Host**，我们只能对"本地标题命中"按 audience 过滤，Host 返回的片段无法分类（边界，已写进矩阵 §6）。

**手机上的返回键：第一次白按、第二次退出应用（P8-1）**。这一轮的事实源不是官方 Web（Web 没有物理返回键），是本仓自己的 **`docs/08-UI与返回交互审计.md`（P0 §1/§2）+ 一条布局事实**（`WorkspacePane.buildStacked` 的窄屏栈式布局：0=工作区列表、1=文件树、**≥2=预览**）。顺着这三处查出三个用户能直接撞上的问题。① **次序反了**：阶梯里"工作区下钻退一层"排在"关文件预览"之前，而单栏下预览**就是第 2 层** ⇒ 按返回只把层数从 2 减到 1，`previewOpen` 却留着：预览确实不在屏幕上了，但之后那一次返回键**白按一次**（屏幕上什么都不变）。现在 `previewIsLayer()` 把"预览此刻是不是**盖在页面上的整页层**"做成判据：整页层排在"下钻"之前（此刻屏幕上最上面的就是它），并排栏形态（多栏右栏/侧栏面板）仍排在页面层级之后——**同一个事实、同一处判定**，不是两套规则。② **预览其实关不掉**：视图里写的是 `this.preview = undefined`，而预览的真值在中枢 `SessionHub.filePreview`、视图那份只是镜像（每次广播都从快照覆盖回来）⇒ 下一次中枢广播（新事件、文件列表、心跳）就把预览**装回来**。中枢的 `closeFilePreview()` 早就写好，却**一个调用点都没有**——这正是本项目反复出现的"通道有、没接"。③ **手机会话页按返回两次才离开应用**：`openSession` 在单栏下既切主区面板又设 `stackPage=CONVERSATION`，而 `STACK_TO_MAIN` **只改 `stackPage`**，主区面板仍是会话 ⇒ 按返回屏幕上什么都不变（白按），再按一次就到根层**直接退出应用**。现在新增 `BackAction.BACK_TO_LIST`（回列表 + **保留会话**，会话仍在 Host 上跑、点一下就能回去），排在二级页之前。**归一化也一起收了**：返回键、页头返回箭头、工作区文件树的返回箭头、诊断/连接页的返回箭头现在都走 `Index.consumeBack()`（**唯一执行点**，此前四处各写一行），浮层复位也只剩 `closeSheet()` 一处（三个 `closeXSheet()` 变成一行委托；顺带修掉结构浮层关闭时清错"凭据忙标志"的错位——**如实说明**：它没有变成已复现缺陷，因为打开路径自己会复位，但它就是"两套复位迟早不一致"的样本）。门禁：fixture **908 → 922 条**，其中新增 **2304 种组合态矩阵**（把手写阶梯写成**数据表**，用一份独立实现去对账 if 链，任何一条分支挪位置都会在某个组合上被抓住）；接线门禁 31 → **32 项**（第 32 项钉的就是"中枢能力必须有调用点"）。**两张门禁都做了归真验证**：只回退“次序 + 会话整页分支” ⇒ 组合矩阵在 **2304 种组合里红了 74 种**，还原即绿；而接线门禁**当场发现自己瞎**——它的计数不剥注释，我写在注释里的 `closeFilePreview()` 让"真调用被换回旧写法"照样判绿，改成"只看代码"后同一归真立刻红（这也是"门禁必须先在已知坏版本上红过一次"的用处）。**仍缺（已登记）**：多栏形态下预览是**右栏面板**时该不该吃返回键，取决于那个面板此刻是否显示——本轮只按"整页层"判（单栏，可证），多栏留给真机 **D40** 确认。

**agent 改了 5 个文件，界面上看不到"产出了什么"（P7-21）**。官方 `dsh-client-ui-deliverables` 有两条槽位，规则写得很清楚："The source is the arguments of **successful `write`, `edit`, and mutating `str_replace_editor` calls**, not the closing prose: **a produced file must be listed whether or not the model remembered to name it**. Reads, unsupported tools, malformed calls, and failed results contribute nothing. Paths keep **first-seen order** and appear **once**." —— 也就是说这份清单**不依赖模型记不记得说**。而我们只有 `deliverables/presented` 事件（取决于模型是否上报）与右栏那份按条目筛的清单 ⇒ agent 改了 5 个文件但没上报时，用户在界面上只能一条条翻工具卡。现在新增：① 零依赖 `appstate/src/main/ets/model/ProducedFiles.ets` 按官方规则**推导**本回合产出（`kind===TOOL` 且 **`toolState===SUCCESS`** 且 `intendedDiffOf()` 非空 ⇒ 取路径；首次出现顺序 + 去重；**失败/被拒/未结算都不算**——把没写成的文件列进产出就是撒谎）；**复用** `ToolDiff.intendedDiffOf()` 而不是重写第二份解析（它对名字/参数 JSON/路径非空/字段类型/提权字段的收窄判据逐条对齐官方）。② `entry/src/main/ets/view/ProducedFilesRow.ets`：回合尾部一行 chip（0 个文件整行不渲染，与官方一致）。③ **点击必须真的到得了文件**：中枢读内容 + 宿主切到工作区页——只读不切在手机上等于"点了没反应"（单栏形态右栏不在屏幕上），这条是本轮唯一穿四层的改动，值得：**假入口是本项目反复踩的坑**。差异登记：官方的 `fitProducedFiles()` 按宽度测量决定显示几个，我们用横滑 + 上限 6 + 文案交代总数（**不假装知道可用宽度**）。fixture **896 → 908 条**；接线门禁 30 → **31 项**；真机判据新增 **D39**。
**目标栏只能"看"，而且空态白占一行（P7-20）**。官方 `dsh-client-ui-goal` 的目标栏是**可操作的**（`GoalBar.d.ts` 的契约原话："resume when paused, edit (inline form in the same strip), and clear. Goal creation lives on the `/goal` command, not here: loading (undefined), no goal (null), and complete goals render nothing"），而我们只有 `refreshGoal`（读）—— 四个动词一个都没有：用户能在网页上暂停/改目标，在这里只能干看着。此外我们**没有相位**（解析只取正文，靠一串候选键名去猜）⇒ 既显示不出"已暂停/受阻"，也无法决定该给"暂停"还是"继续"；空态还白占一行（没目标时写"该会话当前没有长期目标"，而官方是**整条不渲染**）。现在：按 `dsh-goal` 的权威类型（`GoalRef{id,revision}` / `GoalSnapshot{objective, phase: active|paused|blocked|complete, blockedReason?, maxGoalRounds}`）逐字投影，写动词用生成表里的参数名（`['agentId','ref']` / `['agentId','ref','request']`，`ref` 必带读到的 `revision`——Host 用它做乐观并发控制）；**实时性**也补上了：官方读的是宿主算好的 `goal` 投影，我们现在既读投影、也吃 `goal/change` 事件（非 clear 分支**带整份快照**，clear 分支清空），两条来源共用一个落点——否则 agent 标成完成/受阻后那条栏会一直显示旧相位。判定全在零依赖 `appstate/src/main/ets/model/GoalBar.ets`：可见性（没有/已完成/正文空 ⇒ 不渲染）、相位文案、**相位未知时一个动词都不给**（猜错就是一次误导性操作）、受阻时正文带上原因；**唯一保留的空态是"读不到"**（把"读不到"说成"没有目标"会让用户以为目标丢了）。fixture **874 → 896 条**；真机判据新增 **D38**。
**失败文案：上游 46 个错误码，我们只写了 13 句（P7-19）**。上游有一张**权威错误码表**（`dsh-api-session-controller` 的 `RemoteErrorDetailsMap`，46 个码，各带 details 形状），而本仓此前只有 `friendlyFailure()` 的 8 个特例加 `friendlyWorkspaceFailure()` 的 5 个，**其余一律兜底 `${code} ${message}`** ⇒ 真机上用户看到的是「session/conflict session "xxx" already has cwd "/path"」这种只有开发者读得懂的句子（不知道该换目录还是该改路径）。现在新增零依赖 `appstate/src/main/ets/model/FailureText.ets`：40 条专门文案 + 本客户端自己的码（`carrier/*` 等）+ 一个 **`REACHABLE_FAILURE_CODES` 可达集合**（从那张 46 码表里挑出与本仓调用点相关的；`subagent/*` 八个没有界面入口就不写——写用不上的话是自欺），中枢那两个函数各自瘦成一行委托。两条纪律被 fixture 钉住：**兜底必须带上原始码并说清"还没有专门文案"**（这样上游多了新码时界面上看得见）、**专门文案不许以 code 开头**。两个码特意"看原文分流"：`session/attachment-invalid`（同一个码覆盖文件回执与图片两条路，处置完全不同）与 `carrier/*`（连接层的 reason 是第一手证据，必须带出来）。**门禁当场抓到一次搬家丢逻辑**：我改成委托时顺手把这两处的特例丢了，对账断言立刻红在 `session/attachment-invalid` 上——这正是"搬进纯模型 + 对账"的价值（搬家最容易丢的就是特例，而它恰好能被对账看见）。fixture **850 → 874 条**；接线门禁 29 → **30 项**。
**模型重试在界面上是"出错了 + 一片空白"（P7-18）**。设备的网络条件最差时，模型请求会失败并触发重试——这正是最需要界面说实话的时刻。上游的载荷是清晰的（`dsh-llm-retry` 的类型逐字：`{mode:'normal', retry, maxRetries, delayMs, failure:{code,message}}` 或 `{mode:'always', retry, delayMs, failure}`；`llm/retry-started` 在等待结束、请求发出前写入），官方也专门做了带**倒计时**的节点（`ModelRetryItem`：`deadline = Date.now() + node.delayMs`，上限 `node.mode === 'normal' ? node.maxRetries : '∞'`）。而我们只绑了一个 `retry` 字段、拼成 `title = "重试 2"`，正文没有来源 ⇒ 用户看到的是**一条"出错了"加一片空白**：不知道在重试、不知道第几次、不知道还要等多久（而这一等可能十几秒）。现在：`EventShape` 按上游类型逐字补五个槽位、`llm/retry-started` 从"被投影丢弃"改成真条目（否则界面永远停在倒计时）、两段文案做成纯函数并被 fixture 钉住（计数行四种形态 + 倒计时三种状态）、新组件 `RetryRow` 给**每秒自己减的倒计时**（只在还没开始重试时开定时器；起点用**事件时间**而不是"现在"，否则事件迟到 3 秒会让倒计时偏大、看起来像卡住）。顺带删掉错误行里那个假的「重试」标签——它长得像按钮却**没有 `onClick`**，点了没反应（真重发走消息行的「重发」，自动重试由 Host 负责）。fixture **840 → 850 条**；接线门禁 28 → **29 项**；真机判据新增 **D37**。
**审批卡上的"假入口"，以及一整套没人用的词汇（P7-17）**。这次轮到审批——安全决策那张卡。官方 `dsh-client-ui-approval` 的动作行里就是**两颗按钮**（源码逐字：`answer("rejected")` 与 `answer("allowed-once")`），因为上游 `ApprovalOutcome` 是闭集（`allowed-once | rejected | cancelled | unavailable`），**没有任何持久授权**。而我们的卡片有第三项「本次会话内均允许」，detail 写着"仅在本客户端生效，Host 侧仍逐次询问"——实现里 `allowAlways` **只写一行旁路日志**，也就是说它**连"本客户端"都没生效**：用户点它是以为"以后别问了"，实际下次照样问。这正是本仓明令禁止的东西（**点了没反应的假入口**），还长在一个安全决策上 ⇒ 直接删掉，回到官方两选。同轮顺出来一件更大的事：`Wire.ets` 里躺着一整套**零调用点**的产品层词汇（`ApprovalDecision{ALLOW_ONCE,DENY,ALWAYS}` / `approvalOutcomeFor` / `decisionIsClientSideOnly` / `approvalAnswer` / `ApprovalAnswerValue`），全靠 barrel 再导出撑着——零消费者门禁按**既有边界**不扫 `Wire.ets`（那是"上游协议词汇表，形状先写全"），所以它们一直没被发现，而其中的 `ALWAYS` 正是那颗假按钮的余毒，留着等于给下一个人留一条"接回来"的歧路。另外：中枢读审批请求原本是就地的第二份实现，改回用既有的 `Wire.projectApprovalRequest()`，并因此多出一条安全默认——**缺 `toolName` 就不摆卡**（交还 Host 链条）：`toolName` 是唯一必填业务字段，没有它卡片说不出"在批准什么"，却仍然有「允许一次」，那等于让用户批准一个自己看不见的操作。10 道门禁全绿；真机判据新增 **D36**（两选 / 下一次同类操作必须再弹卡 / 缺工具名的卡按缺陷记 / 危险操作主选是拒绝）。
