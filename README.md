...
- **P9-7 交付物分享成真**：交付物卡的「分享」不再是假禁用——沙箱内的文件**分享文件本体**、沙箱外如实降级为分享**路径**（按钮文案随之变「分享路径」）、定位不到则禁用并写明原因；「保存」仍禁用（平台确实没有"另存为"）。
- **P9-6 会话滚动位置**：去设置页/看文件预览再回到**同一个**会话时，回到你离开的位置（切到**别的**会话仍按既有决定贴到底部）。
- **P9-5 `@` 引用菜单钻取**：选中目录后菜单**不关**、接着列下一层并显示面包屑（`工作区 › src › …`）；顺带修好**引号形态**——含空格的路径（`@"my dir/`）此前会被当成"引用结束"，查询词还带着引号发给 Host，等于永远补全不了。
- **P9-4 命令参数分流**：需要参数的命令不再"选中即执行"（此前必然被 Host 拒绝并弹英文 usage），而是把 `命令 + 空格` 填进输入框并说明理由；判据取 Host 的 `input.hint`，不写命令白名单。
- **P9-3 正文文件提及可点**：终答里行内代码写的路径，若命中本回合产出的文件就变可点（官方 `MarkdownFileMentions` 的规则：只认行内代码、用真实文件词汇解析、认不出来保持普通代码）——此前它只是等宽文字。
- **P9-2 原图预览（灯箱）**：点缩略图（单图与方图都行）打开全屏原图预览，几何与无障碍文案逐字对齐官方 `ImageLightbox`——此前多图只是"就地放大到 240"，**单图压根点不动**。
- **P8-6c 错误提示的生命周期**：过期的错误提示会自己消失（动作成功即清、连接级只有真正连上才清）——此前一次"发送被拒"之后成功跑完一整轮，那条红字还挂着，用户只能怀疑已经成功的事。
- **P8-7 技术债与守卫**：`accessSync` 会抛异常而我们 24 处当它返回 false —— 核心树校验的"关键件哨兵"因此会把可读失败变成未处理异常；统一走 `FileProbe`，并清掉 3 处 API 弃用告警（模块编译从 6 条告警到 0 条真问题）。
- **P8-4b 操作区不被裁**：输入区工具行 / 会话头视图切换行 / 时间总览统计行改成**可换行**（与官方 composer 的 `flex-wrap` 同构）——此前它们装满固定宽度控件，手机上最右边的「模型」chip 会被**静默挤出屏幕**。
- **P8-4 浮层关闭语义**：改了一半的设置被拖拽关掉时，现在会明确说"已放弃未保存的修改、没有写入 Host"（此前静默丢弃 ⇒ 用户以为生效了）；没改过则一个字都不说。
- **P8-3 每会话草稿**：草稿按**会话**保存与取回（官方契约：草稿是会话视图状态的一部分），并**落盘**——修掉"在会话 A 写一半、切到 B 顺手发送 ⇒ 那段话发给了 B"，以及"应用一重启没发出去的字全没了"。
...
**验收口径**：`DONE` 只表示"实现侧完成"，设备验收是另一根轴——逐项判据与真机读数在 [`docs/device-validation.md`](docs/device-validation.md)。**设备验收已于 2026-09-16/17 在 HUAWEI Mate 70 Pro+ 上跑过一轮**，抓出 32 条缺陷（原始读数与 F1–F33 台账见 [`docs/device-validation-readings.md`](docs/device-validation-readings.md)）。

**当前状态**（2026-09-17 盘点，可机器复核）：**已修已验 6 条** / **已修待验 16 条** / 部分已修 2 条 / 待定位 1 条 / 待修 1 条 / **核心版本不具备 4 条**（定时任务、取消归档、撤销、待决跨重启——已逐条核实无端点，**不造假入口**）。

- 怎么修、为什么这么修：[`docs/80-修复与优化方案.md`](docs/80-修复与优化方案.md)（含 UI 对标官方的口径表与逐项结论）；
- 接上设备怎么验：[`docs/81-待真机复验清单.md`](docs/81-待真机复验清单.md)（甲/乙/丙/丁/戊五档，甲档约 15 分钟跑完）。
