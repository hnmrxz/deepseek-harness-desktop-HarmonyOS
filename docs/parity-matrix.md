...
| **真机待验收** | `register()` 的钩子跑在 Node 的**独立线程**里；端侧嵌入式运行时是否允许起线程，本环境无法证明。故 `installUndiciNameHook()` 失败时**只降级、不阻断启动**，并把"真机确认钩子注册成功日志 + 真机抓一次网页"列入 `docs/device-validation.md` |
