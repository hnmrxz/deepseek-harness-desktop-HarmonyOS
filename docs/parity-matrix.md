...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.67 | 2026-09-16 | **P9-7 交付物上的「分享」是个假按钮——而它本来能做成真的**。此前「保存 / 分享」可见但禁用，理由写着“平台只支持分享**文本**、不支持文件本体”，而 SDK 读清后**这条理由不成立**：`systemShare.SharedRecord` 支持 `uri`（文档逐字 “Either content or uri must be set”），沙箱内的路径可经 `fileUri.getUriFromPath()` 变成系统可授权的 URI；顺带暴露一个**零调用点**的 `shareText`。官方 `dsh-tool-present` 的 `PresentedFile.path` 逐字：“Original absolute path or path relative to the Session working directory.” ⇒ 先归一成绝对路径再判定。现在 `model/ProducedFiles.deliverableShareTarget` 分三档：沙箱内 ⇒ 分享**文件本体**；沙箱外 ⇒ **如实降级**为分享**路径**（按钮文案随之变成「分享路径」——写「分享文件」却只发路径就是骗人）；定位不到 ⇒ 禁用并**写明原因**。平台新增 `shareFile()`，宿主四层接线到卡片。**「保存」仍禁用**（平台确实没有“另存为”）。fixture 1111 → **1128 条**（归真：去掉沙箱边界判定 ⇒ 6 红）；接线门禁 41 → **42 项**；真机判据新增 **D54**。**仍缺**：图片分享需先落沙箱临时文件（平台层无“写文件”）；“另存为”能力缺口仍在 |
| v1.66 | 2026-09-16 | **P9-6 去看一眼设置再回来，正在读的那段历史没了**。进会话"一律贴底"这条规则在"新会话/切会话"上是对的（P1 的明确决定：切会话 = 看最新），但它把"去设置页看一眼、点开产出文件再回来"也一并贴了底 ⇒ 长会话里要重新往下翻很久。现在 `model/Follow` 增加**只服务"离开又回到同一个会话"**的位置记录（`ScrollSpot` / `rememberScrollSpot` / `scrollRestoreTarget`）：四条判据缺一不可（记录里是这个会话 / 下标在范围内 / 用户没自己滚过 / 列表已装下那一条，数据异步到时在 `onItemsChanged` 每次再试），且"贴底跟随时离开 ⇒ 不记"（回到底部是默认动作）。**刻意不按会话各记一份**——那会与"切会话 = 看最新"的既有决定冲突。记录由宿主 `Index` 保管（面板在页面切换时销毁重建；与草稿袋同类），门面两个成员读写。fixture 1098 → **1111 条**（归真：恢复判据改成永远 -1 ⇒ 2 红）；接线门禁 40 → **41 项**；真机判据新增 **D53**。**仍缺**：只记条目下标、未记像素偏移；跳变/平滑观感待真机 |
| v1.65 | 2026-09-16 | **P9-5 `@` 菜单选目录就关，含空格的路径永远补全不了**。两处缺陷：① 官方选中**目录**时菜单**不关**（`onPick` 返回 `{text, continue: true}`）接着列下一层，我们一律 `panel=''` ⇒ “目录可钻取”形同不存在；② 官方 `formatFileMention` 的引号形态（含空白 ⇒ `@"path"`，目录故意不闭合引号）在我们的词法里是坏的——“见空白就当引用结束” ⇒ 敲 `@"my dir/` 面板立刻收起，且**发给 Host 的查询词带着引号**（匹配不上）。现在：`TriggerToken` 增加 `quoted` 标志（与官方候选源签名同形）、`detectTrigger` 支持 `@"…"`（引号内空白是路径的一部分）、`referenceQueryOf()` 剥引号；选中目录 ⇒ 菜单保持开 + `drilled` + 重列；`referenceCrumbs(query, drilled)` 实现官方 `crumbsFor`（根标签 `crumb.root`=**工作区**）；行信息逐条对齐官方 `fileCandidate`/`sessionCandidate`（目录名带尾斜杠、父目录仅未钻孔时、会话行「位置 · 时间」+ 官方桶词 `{n}分钟`、同工作区不写位置、无 cwd 写「（无工作目录）」），为此补读官方 payload 的 `createdAt`/`sameWorkspace`。fixture 1056 → **1098 条**（归真：去掉钻取判据 ⇒ 2 红）；接线门禁 39 → **40 项**；真机判据新增 **D52**。**仍缺**：面包屑不可点、`cwd` 未做 `~` 缩写 |
| v1.64 | 2026-09-16 | **P9-4 命令面板“选中即执行”：要参数的命令必然被 Host 拒绝**。Host 的 `CommandDescriptor.input.hint` 就是“这条命令要一段自由文本”的权威信号（我们早就投影成 `CommandCandidate.args`），可面板只把它当副标题、甚至被 `description` 挤掉，且不论要不要参数**都直接执行** ⇒ 选 `/feedback` 得到 Host 那句英文 usage，面板关了、输入框空着。官方 `CommandDecoration` 逐字：“the decoration replaces only the bare menu-pick/enter with a popup whose onSelect **typically submits a completed line back through command.execute**” ⇒ **永远不发出一次已知不完整的调用**。现在：`InputTrigger.commandNeedsInput/commandPickAction/commandDraftText/commandInputNotice` 四条纯函数，面板分流（要参数 ⇒ 填进输入框 + 三要素说明；不要参数 ⇒ 照旧执行），命令行把参数提示与说明**各占一行**。**边界事实（决定修法）**：`popupSelect` 没有协议面（Host descriptor 只有 `{name, description, input?}`；官方注释写着贡献 “lives entirely on the client (no host descriptor)”），它的选项列表我们按命令各自的具体入口实现（权限 chip / 模型选择 / 目录选择），**不另造通用弹层**。fixture 1043 → **1056 条**（归真：改回“一律直接执行” ⇒ 1 红）；接线门禁 38 → **39 项**；真机判据新增 **D51** |
| v1.63 | 2026-09-16 | **P9-3 正文里的文件路径只是等宽文字，点不动**。官方 `MarkdownFileMentions`（`dsh-client-ui-primitives`）逐字规定：文件提及挂在**行内代码**上，由 owner 用**自己的真实文件词汇**解析，认不出来就"**stays inert code**"——"the renderer never guesses at what looks like a path"。我们此前 `\`src/a.ts\`` 只是等宽文字，用户只能去过程流里翻工具卡。现在：零依赖 `resolveProducedMention(files, token)`（三条规则：全路径 → `/token` 结尾 → basename；**歧义返回空串**——两个 `index.ts` 就不猜，猜错会打开另一个文件）、`MarkdownRenderer` 增加 `mentionPath`/`onOpenMention` 回调位（行内代码解析成功时下划线 + 品牌色，无障碍里带完整路径 = 官方 `title` 的等价物）、词汇表在 **`TurnView`** 给（与回合尾部那行 chip **同一份来源**，打开动作复用 P7-21 的 `onOpenProduced`）。扁平台账流不传词汇表（它没有"回合"概念，硬给等于猜）。fixture 1030 → **1043 条**（归真：歧义取第一个 ⇒ 2 红）；接线门禁 37 → **38 项**；真机判据新增 **D50** |
