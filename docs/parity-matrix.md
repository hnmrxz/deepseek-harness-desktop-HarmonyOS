...
| `user-questions` | **输入区接管已做（P7-2）**（与 `approval` 同一批：官方 `dsh-client-ui-user-questions` 也 inject `conversation.composer`），单条提问同样只在本会话输入区出现。**仍缺**：官方提问卡是**整组**呈现 —— `nav.prev/next`（上一题/下一题）、`nav.minimize/maximize`（收起/展开）、`nav.cancel`（放弃整组问题）、`option.recommended`（推荐标记）、`action.skip`（跳过本题）、`custom.placeholder`（输入你的答案）、以及**计划复核**（`plan.header: 计划待审` / `plan.approve: 确认执行` / `plan.decline: 拒绝` / `plan.discuss: 去聊天里说`）；我们的 `PendingItem` 是**单条**模型，没有"整组"这个概念 ⇒ 要对齐得先让投影带上问题组 | P1：同上（与 `approval` 同一批） |
