...
| `attachment` 附件呈现 | 动态附件呈现：输入区、消息图、轨迹图三类槽位 | 输入区：`attachLocalFile` / `attachWorkspaceFile` / `removeAttachment` / `clearAttachments`；消息图与轨迹图：`projectImageBlock` → `TrajectoryItem.images` → `SessionHub.imageUrlOf`（`session/attachment` 读字节 + 会话作用域缓存） | 输入区 `Composer` 附件条 + **`view/MessageImages.ets` 画廊**（单图大图 / 多图 64px 方图，点击就地放大；挂在 `MessageRow`，**对话与轨迹两个视图共用**） | `fileUploads/*`、**`session/attachment`** | PARTIAL | PARTIAL | PARTIAL | PARTIAL | PARTIAL |
...
| `attachment` | **消息内图片与轨迹图片已接（P0-3）**：官方 `dsh-client-ui-attachment` 往三个槽位注册呈现（`conversation.input.attachments` / `conversation.message.images` / `conversation.trajectory.images`）。三槽位的**数据形状**已从官方源码读齐：图片块是 `{type:'image', attachment:{attachmentId, mediaType, bytes, width, height, name}}` —— **只有不透明引用**，字节必须另走 `session/attachment`（官方 `ISession.readAttachment` → `remote.session.attachment`，回复 `{attachment, data: base64}`）。我们此前把图片块降级成正文里的字面 `[image]`（`splitBlocks` 的非文本块兜底分支）⇒ 真机上用户看到的就是这五个字符。现在：① 投影收集引用（`projectImageBlock`，只认 `attachment`，与官方 `contentParts` 同判据）；② 中枢读字节并缓存（`imageUrlOf` / `imageErrorOf` / `retryImage`，键 = `会话 id:附件 id`——**附件 id 内容寻址而授权是会话作用域**，只按附件 id 缓存会把"某会话读过"当成"所有会话可读"；上限 24 张，断开连接即清空）；③ 呈现 `view/MessageImages.ets`（官方几何：单图 `singleFit` 最大边 240 / 比例夹 [0.25,4] / **绝不放大小图** / 超界比例保留信息起点一侧；多图 64×64 方图 + 间距 10vp；圆角 16；按角色左右对齐），几何判定的**唯一真值**在零依赖 `model/MessageImage.ets`（被 fixture 钉住）。**真 Host 取证**：`tools/check-core-loop.mjs` M2e 用假附件 id 打真 Host ⇒ `session/attachment-invalid` ⇒ **端点确实实现着**（不是"表里有"的推测）。**仍缺**：① **输入区发送图片**（官方 `conversation.input.attachments` 槽位；需要平台侧"选图 → 取字节"，本仓 `platform` 目前只有文档选择器）；② 官方点击缩略图开**灯箱**（`ImageLightbox`），我们是**就地放大**（灯箱要页面级遮罩 + 返回键接管，而页面根只有**一个** sheet 宿主，见 `Index.ets` 的 `sheetKind()`）——结果等价、交互不同，如实登记；③ 图片**另存 / 分享**（平台无文件保存能力，与 `deliverables` 同批）；④ `nav.minimize/maximize` 式的卡片收起（官方无此物，仅我们无）。**真机验收**：`docs/device-validation.md` D29 | P0-3 已做消息图/轨迹图；发送图片与灯箱待排 |
