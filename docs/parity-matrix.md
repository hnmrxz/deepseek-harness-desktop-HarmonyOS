...
| `conversation` | ① 回合渲染已落地（对话视图按回合 + 过程分组折叠/展开），但**轨迹视图仍是条目级台账**——§8 的"统一模型"目前只在对话视图生效 ② **sticky-follow 仍与会话滚动耦合**（§8 要求独立管理）：`onVisibleRange` 用行数判贴底，行单位随视图变化（已抽 `rowCount()` 收敛，但跟随状态本身仍挂在滚动的可见区间上）③ 回答本身尚不可折叠（只折叠了过程分组）④ PC/2-in-1 列随 `layout` 的缺口 | P1 已做：回合模型（15 条断言）+ 对话视图按回合渲染 + 行数单位收敛。下一步：轨迹视图也按回合组织（或明确"它就是全量台账"并在文档里定死）、sticky-follow 独立化 | | P1：Conversation 重构（先做模型，再改视图） |
