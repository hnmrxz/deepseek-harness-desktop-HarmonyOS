...
| id | 缺什么（对等差距） | 下一步（归属） |
|---|---|---|
| `layout` | ① 无拖拽调宽手柄（官方 AppFrame 有 drag handles）；`decideLayoutWithDetail` 已把"用户想要的宽度"这条路径做好并有 fixture，但**没有 UI 去产生这个宽度** ② 详情栏宽度记忆未落 ⑤ **平板（DOUBLE）目前与手机一样用半模态 Sheet**；计划 §12 说的"平板侧边浅层面板"尚未做（三栏已经是真右栏） ③ **待决（需真机）**：D3 §2 只按宽度判定 ⇒ **手机横屏（800vp 宽）会落成双栏**；要不要加高度/方向子句，看真机效果后定 ④ **输入模态事实已接通**（P3）：此前 `hasPointerInput = keyboardLikely` 被**同时**当作 hasPointer 与 hasKeyboard，于是手机插鼠标/平板配触控笔画不出 hover 与右键、2in1 拆掉键盘仍被当成有键盘。现在三个来源按明确优先级合成（`model/InputFacts`，37 条断言）：① 系统**真实枚举**输入设备（`multimodalInput.inputDevice`，无权限要求，含热插拔重枚举）② 运行时事件证据（悬停/按键，只增不减）③ 设备形态猜测（兜底，保证不比旧行为差）；映射依据 SDK 的 `SourceType` 字符串联合，且 **`touchscreen` 刻意不算指针**（触控没有悬停与右键，其入口是长按）。决策侧不再硬编码 `false`。**仍缺**：`pointerRich` 现在**算得对但仍无消费点**——建议的消费点是"有指针时消息操作条改为悬停显现"，但那是会**藏起可点入口**的改动，手感与触控侧的影响需真机判断，故不在此拍脑袋做 | P2：拖拽调宽 + 宽度记忆；P3：把 `readDeviceFacts().keyboardLikely` 接进 `LayoutInput` |
