...
| `hdsh-composer-drafts` | **每会话草稿已做（P8-3）**：按会话保存/取回输入草稿（官方 `contract/views.d.ts` 逐字："Composer draft (persisted; survives session switches **and reloads**)"）。**已实现**"跨会话切换"这一半（`model/ComposerDrafts` + 宿主 `switchDraftContext`）；**仍缺**后半截"跨重载"：应用重启后草稿会丢——需要 `platform/LocalPrefs` 增加一个字符串键 + JSON 编解码，并处理"启动时序"（输入框首次渲染早于预读取回）。⇒ 下一步：`LocalPrefs.saveDrafts/loadDrafts` + 页面 `aboutToAppear` 之后的一次性恢复 | 
