...
本目录是 **HDSH**（DeepSeek Harness 鸿蒙客户端）项目的文档基线。开发任务书（D1）是范围、架构与验收的唯一权威；其余文档是其展开或配套。
...
| **D8** | [`70-上架合规自查.md`](70-上架合规自查.md) | **上架自查**：包结构与形态、**权限清单（2 项，逐项给代码证据）**、数据与网络姿态（无遥测/数据不出沙箱）、受限能力红线自查、HAP 体积构成与取舍、**release 打包链路与签名的现状**（本轮修通了 release 打包；release 证书仍缺）、AGC 侧待办、可复跑的复查命令 | **准备提交上架前**；每次改动权限、包结构或 build 配置后 |
...
- **临时过程产物一律只落 `dist/`（已 gitignore）**：本地调试的 `home*` / `sandbox` / 截图 / 日志 / 探针输出都属于可再生产物，随用随清（本轮已清 73 个文件 + 21 个目录 ≈ 8.1 MB）。
...
| 角色 | 顺序 |
|---|---|
| **所有人（先读）** | **D6 全文**（目标与架构的权威更正） |
| 项目负责人 / 评审人 | **D6** → D1 全文（注意其远程客户端定位已被更正）→ D1 §12 待决事项 → D4 界面完成度对照表 |
| 端侧运行时 / 打包 | **D6 §4~§6** → D4 POC-11/POC-12 → `.research/ref-harmony/specs/ARCHITECTURE.md`（社区反面对标） |
