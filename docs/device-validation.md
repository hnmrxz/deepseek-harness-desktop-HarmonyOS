...
四条类型白名单与魔数嗅探）与"这条链路在真 Host 上走到哪一步"（核心闭环 M2f：本机是 SKIP，
原因是宿主探针说开发机缺 HAP 的 libs 映射）。**证明不了**的是设备上到底能不能出图。

**先做第 0 步（不做这一步，后面几条的结论都会指错方向）**：
核心页 →「原生件加载」或**复制诊断报告**里的「原生件」一行，看 `sharp` 是"已加载"还是"未加载 + 原因"。
若为未加载，则整个图片链路在设备上**本来就不可能可用**——按 E384h 重新物化核心
（`node tools/pack-core.mjs`，不带 `--skip-install`；完成后 `node tools/pack-core.mjs --check-sharp` 必须变绿）
并重打核心包，**不要**去查"图片格式"。
