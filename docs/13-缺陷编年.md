...
> **这是什么**：2026-09-14 起逐轮的"缺陷 → 官方依据 → 修法 → 门禁 → 仍缺"记录，**按时间倒序**。
> 它**不是**需求文档，也**不是**第二套规范：当前状态看 [`07-当前状态与缺口.md`](07-当前状态与缺口.md)，
> 能力对等与缺口登记看 [`parity-matrix.md`](parity-matrix.md)，协议事实看 10/11/12 三份，
> 运行架构的历史结论看 [`50-端侧核心运行架构.md`](50-端侧核心运行架构.md)（每轮的 E 编号在那一份里）。
>
> **为什么单独成文**：README 每轮追加一段，最终长到 200 余行且把"怎么装、怎么读"挤到了下面。
> 记录本身有价值（它是"为什么这么改"的唯一出处），所以**搬到这里而不是删掉**；
> 新增轮次请写进本文件，README 只留结论与入口。

## 本轮之前的README正文（原样保留）

## P9-7 交付物上的「分享」是个假按钮——而它本来能做成真的

**缺陷**：交付物卡片上的动作行里，「保存 / 分享」两项是**可见但禁用**的，理由是"平台只支持分享**文本**、不支持文件本体"（写在无障碍文案里）。这一轮把 SDK 读清楚后发现**理由不成立**：`systemShare.SharedRecord` **支持 `uri`** —— 文档逐字："method to obtain the file URI from the sandbox path. **Either content or uri must be set.**"，也就是**沙箱内的文件路径可以经 `fileUri.getUriFromPath()` 变成系统可授权的 URI**，接收方打得开。于是我们不但让一个用户看得见的能力白白禁用着，还顺带留下一个**零调用点**的 `shareText`（`platform` 里早就写好了、没人用）。

**官方依据**：`dsh-tool-present` 的 `PresentedFile.path` 文档逐字："**Original absolute path or path relative to the Session working directory.**" ⇒ 路径可能是绝对的、也可能相对会话工作目录，归一成绝对路径之后才谈得上"能不能分享"。

**修法（分三档，各说各话）**：新增 `model/ProducedFiles.deliverableShareTarget(rawPath, sessionCwd, sandboxRoot)`：
- **沙箱内** ⇒ `SHARE_TARGET_FILE`：平台把绝对路径转成 `fileUri` 交给系统分享面板（分享**文件本体**）；
- **沙箱外**（用户经系统选择器挑的工作区）⇒ `SHARE_TARGET_TEXT`：**如实降级**为分享**路径文本**，按钮文案变成「分享路径」（`shareActionLabel`：写「分享文件」却只发路径就是在骗人），并在无障碍里说明为什么降级；
- **定位不到**（没有路径 / 相对路径但不知道会话工作目录 / 拿不到沙箱根）⇒ `SHARE_TARGET_NONE`：禁用，但**必须写明原因**（假入口是本仓禁止的形态）。
平台层新增 `shareFile()`（`fileUri.getUriFromPath` + `utd: general.file` + `SharedData`/`ShareController`，失败返回 false 让调用方给回执）；宿主接线四层（`Index` 算档并拉起面板 → `MainShell` 门面 → `ConversationPane`/`TurnView` → 卡片）。**「保存」仍然禁用**——平台确实没有"另存为"，这一条理由是真的，继续写在无障碍里。

**门禁**：fixture **1111 → 1128 条**（17 条：三档各自的情形 + 相对路径归一 + 双斜杠 + 没有沙箱根 + 按钮文案）；**归真验证**：去掉沙箱边界判定（一律当"能分享文件"）⇒ **6 条红**。接线门禁新增「交付物分享」项（41 → **42 项**）。真机判据新增 **D54**。

**仍缺（已登记）**：图片（消息里/灯箱里）的分享需要先把内联字节写成沙箱临时文件——本仓平台层还没有"写文件"能力；「另存为」仍然没有（官方 web 是浏览器下载，端侧需要系统文件保存能力）。

---

## P9-6 去看一眼设置再回来，正在读的那段历史没了

**缺陷**：进会话**一律贴到底部**（`aboutToAppear → scrollToLatest()`）。这条规则在"新会话/切会话"上是对的（会话切换的意图是看最新，那是 P1 的明确决定），但它**同时**把另一种情形也一起处理了：用户正在一段长会话里往回读，去设置页看一眼、或点开一个产出文件再回来 —— 位置没了，得重新往下翻很久。这两件事的意图完全相反，却共用了同一条"贴底"。

**修法**：`model/Follow.ets` 增加一个**只服务"离开又回到同一个会话"**的滚动位置记录（`ScrollSpot`，`rememberScrollSpot` / `scrollRestoreTarget`）。四条判据缺一不可：
① 记录里就是**这个**会话（换会话仍走"看最新"——**本仓既有决定不动**，所以这里刻意不做"每会话各记一份"）；
② 记的下标还在列表范围内（越界会让 `scrollToIndex` 抛）；
③ 用户**还没有自己滚动过**（他自己滚了就别把他拽回去）；
④ 列表已经装下那一条（数据是异步到的，没到就先等——`onItemsChanged` 每次再试）。
另外"贴底跟随时离开 ⇒ 不记"（回到底部本来就是默认动作，记了会与 follow 打架）。

接线：宿主（`Index`）持有那份记录（`ConversationPane` 在页面切换时会被销毁重建，存它自己身上等于每次回来都丢——与草稿袋同一类"属于会话的界面状态"）；门面给两个成员（`scrollSpot` 读、`onRememberSpot` 写）；面板在 `aboutToDisappear` 记、在 `aboutToAppear` 与 `onItemsChanged` 里按判据恢复。

**门禁**：fixture **1098 → 1111 条**（13 条：记录三种 + 恢复八种 + 边界两种）；**归真验证**：把恢复判据改成"永远返回 -1"（= 修前行为）⇒ **2 条红**。接线门禁新增「会话滚动位置」项（40 → **41 项**）。真机判据新增 **D53**。

**仍缺（已登记）**：只记了"顶部条目下标"，没记条目内的**像素偏移**（同一行很高时回来会看到该行顶部而不是原来那一屏）；`scrollToIndex` 的观感（跳变 vs 平滑）需真机看后定。

---

## P9-5 `@` 菜单选目录就关了，含空格的路径永远补全不了

**缺陷（两处，都在 `@` 引用这一条链上）**：
① **"目录可钻取"在界面上等于没有**：官方选中**目录**时菜单**不关**（`onPick` 对目录返回 `{text, continue: true}`），接着列那个目录的内容；而我们一律 `panel = ''` ⇒ 选一个文件夹之后菜单就关了，用户得再点一次 `@`，而那时补全状态已经不在。
② **引号形态（`@"path"`）是坏的**：官方 `formatFileMention` 逐字规定"路径**含空白** ⇒ 写 `@"path"`（目录故意不闭合引号，留给下一次补全继续钻）"，而我们的词法**见到空白就当引用结束**（`detectTrigger`：`@` 之后有空格 ⇒ 不是引用）⇒ 敲 `@"my dir/` 面板立刻收起、候选列不出来，更要命的是**发给 Host 的查询词带着引号**（`"my dir/`）⇒ 路径匹配不上。含空格的路径正是引号语法唯一存在的理由。

**修法**：
- 词法层（`model/InputTrigger.ets`）：`TriggerToken` 增加 **`quoted` 标志**（与官方候选源的 `quoted` 同形）；`detectTrigger` 支持引号形态——`@"` 之后到闭合引号（或行尾）都是查询词，**其中的空白是路径的一部分**；闭合引号算 token 结尾（"选中即替换"不会留下尾巴）。新增 `referenceQueryOf()` 把引号剥掉（官方 query 里不含引号，引号是单独标志）。
- 钻孔：命令面板同理，引用面板选中**目录**时 `panel` 保持 `'ref'`、`drilled = true`、并把下一层的查询词交给 Host 重列；用户自己敲字则 `drilled = false`。
- 面包屑：新增 `referenceCrumbs(query, drilled)`（官方 `crumbsFor` 的等价实现，根标签逐字取 `crumb.root` = **工作区**）；面板标题在钻孔时显示 `工作区 › src › components`。官方注释写明了为什么只有钻孔才有面包屑："a path the user typed carries its own context in the draft, while a drill **replaced the text they were reading with a deeper one and owes them the way back**."
- 行信息（官方 `fileCandidate` / `sessionCandidate` 逐条对齐）：文件行名字取末段（**目录带尾斜杠**）、父目录**只在未钻孔时**显示；会话行显示「位置 · 时间」——同工作区不写位置、没有 cwd 写官方的「（无工作目录）」、时间用官方引用命名空间自己那套桶词（`刚刚` / `{n}分钟` / `{n}小时` / `{n}天` / `{n}个月` / `{n}年`，与 `Present.relativeTime` **同桶不同措辞**——官方字典注释写明了"分桶共用、措辞各插件自带"）。为此 `ReferenceCandidate` 增读官方 payload 里本就有、我们此前没读的两个字段：`createdAt` 与 `sameWorkspace`。

**门禁**：fixture **1056 → 1098 条**（42 条：引号词法 7 + 钻孔查询词 5 + 面包屑 5 + 时间措辞 9 + 文件行 5 + 会话行 5 + 既有引用断言）；**归真验证**：把面包屑的钻取判据去掉 ⇒ **2 条红**。接线门禁新增「引用菜单钻取」项（39 → **40 项**）。真机判据新增 **D52**。

**仍缺（已登记）**：官方那种"钻取时按段高亮/可点的面包屑"（我们只显示路径文本，不能点某一段跳回去）；会话行的位置用了原始 `cwd`（官方有 `abbreviateHomePath` 把 `$HOME` 缩成 `~`，需要 Host 的 `home`，我们还没取）。

---

## P9-4 命令面板"选中即执行"：要参数的命令必然被 Host 拒绝

**缺陷**：命令面板（`CommandList`）此前**无论命令要不要参数都直接执行**。Host 的 `CommandDescriptor` 里 `input.hint` 就是"这条命令要一段自由文本"的权威信号（我们早就投影成 `CommandCandidate.args` 了），可面板把它**当成副标题用**，甚至被 `description` 挤掉。于是：选 `/feedback`（hint 写着要一段反馈文字）⇒ 立刻发给 Host ⇒ 被拒，界面上出现 Host 那句英文 usage（`Feedback text is required. Usage: /feedback <text>`），**面板关了、输入框空着**——用户会以为命令坏了。（这条缺陷的取证在 P7-13 的真 Host 闭环里就出现过：M2g 那次 `/feedback` 就是为了确认回执形状。）

**官方依据（逐字）**：`dsh-client-ui-commands` 的 `CommandDecoration` ——
> "the decoration replaces only the bare menu-pick/enter with a popup whose onSelect **typically submits a completed line back through command.execute**."

也就是说官方**永远不发出一次"已知不完整"的调用**：需要参数的命令，裸选时先开一个 popup，选中后再把**补完的一行**交回去执行。另有一条边界事实（决定了我们怎么做）：`popupSelect` **不在协议里** —— Host 的 `CommandDescriptor` 只有 `{name, description, input?}`，官方那句注释也写着贡献"lives entirely on the client (**no host descriptor**)"，选项来源是各业务包自己的协议。

**修法（最小且诚实的等价物）**：`model/InputTrigger.ets` 增加四条纯函数——`commandNeedsInput(args)`（判据取 Host 的 hint，**不写命令白名单**：目录是 Host 给的，抄一份到客户端必然过期）、`commandPickAction(args)`（`run` / `draft`）、`commandDraftText(name)`（`/name ` + 一个空格——词法要求）、`commandInputNotice(name, hint)`（三要素：为什么没执行 / 参数长什么样 / 下一步）。命令面板按它分流：要参数 ⇒ 填进输入框 + 说明；不要参数 ⇒ 照旧执行。顺带修掉行里的显示问题：**参数提示与说明各占一行**（此前 `detail` 一非空就把 hint 吞了，用户根本不知道这条命令要参数）。

**登记为能力边界（并写清依据）**：官方的 `popupSelect` 机制本身**不追平**——它没有协议面（client-only），选项来自各业务包；我们已有的等价物是按命令的具体入口（权限 chip、模型选择、目录选择都在输入区/设置里），而"裸命令缺参数"这一类的兜底就是本轮的 draft 分流。

**门禁**：fixture **1043 → 1056 条**（13 条：权威信号三种、动作分流两种、插入文本三种、提示文案四要素 + 无 hint 不留残句）；**归真验证**：把 `commandPickAction` 改回"一律直接执行" ⇒ **1 条红**。接线门禁新增「命令参数分流」项（38 → **39 项**）。真机判据新增 **D51**。

---

## P9-3 正文里写"我改了 3 个文件"，可那几个路径只是等宽文字

**缺陷**：模型在终答里写 `\`src/utils/format.ts\`` 时，那一格只是**等宽文字**——点不动。而官方 `dsh-client-ui-primitives` 的 `MarkdownFileMentions` 早就规定了这件事：**行内代码里的文件提及是可点的**，点击打开那个文件。于是我们的用户看到"我改了 3 个文件"，只能去过程流的工具卡里一个个翻——而回合尾部那行 chip（P7-21）明明已经知道是哪些文件。

**官方契约（逐字，两句话定了全部设计）**：
> "File-mention affordance for inline code: the owner resolves an authored token to the file it names, using **its own vocabulary of real files** — **the renderer never guesses at what looks like a path**."
> `resolve(value)`："…or **undefined when the token names no known file — it then stays inert code**."

① 可点的只有**行内代码**；② 认不出来的一律保持普通代码（不许"看着像路径"就变可点）；③ 词汇表由 **owner** 给 —— 官方把它挂在 turn-tail owner 上（`slots.d.ts` 的 `fileMentions`），也就是**回合**那一层。

**修法**：① `model/ProducedFiles.ets` 增加 `resolveProducedMention(files, token)`：三条匹配规则（全路径相等 → `/token` 结尾 → basename 相等），**歧义时返回空串**（两个 `index.ts` 就不猜——猜错会打开**另一个**文件，而用户以为自己点对了）；超长 token、大小写不符、本回合没产出的路径一律 inert。② `MarkdownRenderer` 增加两个回调位（`mentionPath` / `onOpenMention`），行内代码解析成功时加下划线 + 品牌色 + 无障碍里带上**完整路径**（官方 `resolve()` 的 `title` 就是完整路径，网页上是悬停提示；ArkUI 的 `Span` 没有 tooltip，就放进无障碍文案）。③ 词汇表在 **`TurnView`** 给：`resolveProducedMention(this.produced(), token)`，打开动作**复用 P7-21 那条链路**（`onOpenProduced`：读内容 + 切到能看见预览的页面）——与尾部那行 chip 同一个出处、同一个动作。

**如实说明**：扁平视图（`ConversationPane` 的台账流）**不**传词汇表 ⇒ 那里的提及保持普通代码。这不是漏做：官方也把 `fileMentions` 绑在 turn-tail owner 上，台账流没有"回合"这个概念，硬给它一个词汇表等于猜。

**门禁**：fixture **1030 → 1043 条**（13 条：三条规则各命中一次 + 歧义不猜 + inert 六种情形 + 全路径优先）；**归真验证**：把歧义分支改成"取第一个" ⇒ **2 条红**。接线门禁新增「正文文件提及」项（37 → **38 项**）。真机判据新增 **D50**。

---

## P9-2 图片点不开：单图压根点不动，多图只是"就地放大"

**缺陷**：官方的 `ImageLightbox`（`dsh-client-ui-attachment`）是**点缩略图打开文档级全屏预览**（遮罩 + `contain` 原图 + 右上角关闭；Esc / 点遮罩 / 关闭按钮都能关；关闭后焦点回到缩略图）。我们此前是"点一下**就地放大**到单图尺寸"，而且**单图压根点不动**（代码里写着"单图本来就是大图 ⇒ 点击无事可做"）——可它在手机上最大只有 240vp，一张截图里的字根本看不清；多图那 64×64 的方图点开也只是放大到 240。**上一轮我把它记成"需要页面级遮罩 + 返回键接管，而页面根只有一个 sheet 宿主"，这个判断是错的**：ArkUI 的 `bindContentCover` 就是**组件自己**能挂的全屏模态（系统提供背景、**系统返回键自动接管**、`onDisappear` 回调），根本不用动宿主链。官方之所以要 portal，是因为祖先节点带 transform/filter 会把固定定位的遮罩困住——在 ArkUI 里 `bindContentCover` 承担了同一件事。

**修法**：① `model/MessageImage.ets` 增加官方的灯箱几何 `lightboxFit()`（三条 CSS 的等价实现：`object-fit: contain` + `max-width: min(100%, 1600px)` + `max-height: calc(100vh - 80px)` ⇒ **只缩不放**，与 `singleFit` 同一条纪律）与三个**逐字取官方中文字典**的无障碍文案（`image.preview` = 原图预览 / `image.closePreview` = 关闭原图预览 / `image.openOriginalLabel` = `{label}，点击查看原图`）；② `MessageImages` 里给每张图（**单图与方图都**）挂 `bindContentCover`，内容区实测尺寸喂给 `lightboxFit`，点内容区或右上角按钮关闭，`onDisappear` 复位状态；③ 删掉原来"就地放大"的那套（`zoomIndex` / `zoomPreview`）。

**如实登记的三处差异**：① 官方灯箱取**原图** URL（缩略图可能是另一张更小的图），我们手上只有 Host 给的那一份字节 ⇒ 灯箱与缩略图同一 URL，差别只有显示尺寸；② 官方是半透明遮罩 + 模糊，我们用 `bindContentCover` 的**主题底色**——因为它的 `backgroundColor` 默认是**白色**（SDK 文档），而手写 `rgba(...)` 遮罩正是 README 第 2 条禁止的做法；③ 关闭按钮用本仓的触控下限 `Sz.TOUCH_MIN`（44）而不是官方 CSS 的 36——手指点不中会让用户困在灯箱里，这条比逐字对齐 CSS 更重要。

**门禁**：fixture **1015 → 1030 条**（15 条：五组几何 + 三条文案 + 两个常量）；**归真验证**：把 `lightboxFit` 的"只缩不放"去掉（允许放大小图）⇒ **1 条红**（"小图不放大"）。设计令牌棘轮当场拦下了新写的裸尺寸（36/18）⇒ 换成 `Sz.TOUCH_MIN` 与 `HarmonyIconSize.NAV`。接线门禁新增「原图预览（灯箱）」项（36 → **37 项**）。真机判据新增 **D49**。

---

## P8-6c 那条红色错误提示不会消失：用户只能问"我到底发出去没有"

**缺陷**：中枢只有一个 `lastError` 槽位（三十多处往里写），而会话页顶上那条状态条是**无条件渲染**的。于是：一次"发送被拒/队列修改失败/创建会话失败"之后，用户改一改再发、**成功跑完一整轮**，那条红字**还挂在那里**。用户的合理疑问正是本仓最怕的那种："我刚才那条到底发出去了没有？"——一条过期的错误比没有错误更糟，因为它让人怀疑已经成功的事。

**依据**：`docs/09-安全与API错误呈现审计.md` §3.4 把这条写成硬要求：「**新一轮成功后清除旧的 transient error**；**持久连接问题只有真正恢复连接后才清除**；同一错误不能因为多个事件重复堆叠」。

**修法**：新增零依赖 `model/ErrorLifecycle.ets`，把错误分成**两种作用域**（判据由**写那句话的地方**决定，而不是事后从文案里猜——猜文案是本仓反复禁止的做法）：
- `connection`（连接级：地址无法解析、令牌无效、事件流终止、连接未建立、当前离线）⇒ **只有真正连上**才消失；
- `action`（动作级：发送被拒、队列修改失败、创建会话失败、目录列举失败…）⇒ 同类动作成功即消失。

中枢侧三处接线：① `setConnectionError()` 成为连接层那 5 处的**唯一入口**（文案与作用域一起写，避免"先写文案、忘了设作用域"）；② 连上（`CONNECTED`）时按规则清理，且**放在开流之前**（否则 `refreshSessions` 的新错误会把旧的又写回来）；③ 发送被接受、一轮**成功**跑完（`turn/end` 且 `reason.kind !== 'error'`）时清动作级——失败收场**不清**，那条错误正好解释了为什么这一轮失败。

**门禁**：fixture **1005 → 1015 条**（10 条：两种作用域 × 三种触发、未知作用域按动作级、空触发不清）；**归真验证**：把规则改成"不看作用域、任何事件都清" ⇒ **2 条红**（"连接级 + 发送成功不该清"当场红）。接线门禁新增「错误生命周期」项（35 → **36 项**）。真机判据新增 **D48**。

**仍缺（已登记）**：动作级错误目前只有三个清理触发点（连上 / 发送成功 / 一轮成功）；"修改队列失败"之后若用户只是改了个设置再成功，那条提示仍会留到下一次发送成功为止——要不要按"任意动作成功"清理，需要真机看观感后定。

---

## P8-7 守卫成了崩溃点：`accessSync` 会抛，而我们当它返回 false

**缺陷**：`@ohos.file.fs` 的 `accessSync(path)` 在类型声明里是 `boolean`，但它**会抛** `BusinessError`：`401` / `13900005`（I/O）/ `13900011`（内存）/ `13900012`（权限）/ **`13900018`（Not a directory）** / `13900020`（Invalid argument）。本仓**24 处**把它当纯布尔谓词用，其中最关键的是 `CoreStore.verifyStagedTree()` 的三个"关键件哨兵"：它们存在的**全部意义**是"解包结果不完整时给一句说得清的失败"，可真当哨兵文件缺失（`…/node_modules` 不存在或不是目录）时，`accessSync` **抛**——那句 `fail('解包结果缺少宿主包 @deepseek-ai/dsh')` 根本走不到，用户看到的是一个未处理异常。这正是本仓反复强调的那一类：**守卫本身成了崩溃点，还把失败原因换成了看不懂的东西**。

**编译器早就说了**：`devecocli build` 在 `hostruntime/core/CoreStore.ets:424/430/438` 报 `Function may throw exceptions. Special handling is required.`——告警是对的，代码是错的。（`docs/02` 的 P8-7「清理 API 24 兼容问题与已知技术债」就是这一条的执行入口。）

**修法**：新增 `hostruntime/core/FileProbe.ets`：`fileExists()` / `dirExists()` 把 `accessSync` 包成"返回布尔"（异常按"不存在/用不了"处理，并用 `hilog.debug` 留下 `code`：真机上"明明拷了却报缺件"时，这一行是唯一能说清原因的东西）。**24 处全部替换**（CoreStore 13 / BundledCore 5 / DshHost 4 / PluginRows 2 / entry 侧 NodeRuntime 与 FolderPicker 各 1）。

**顺带清掉三处 API 兼容告警**：① 两处**全局无参 `getContext()`**（已 deprecated）——`OpenLink.openExternalUrl` 改成**由调用方传入上下文**（组件用 `getContext(this)`，`platform` 层本来也拿不到组件上下文），`WindowRegistry.currentWindowId()` 则**直接删掉**（它零调用点，见下）；② `SecretStore.toBytes` 里 `TextEncoder.encode()`（since 7 / deprecated since 9）改成官方 `@useinstead` 指的 `encodeIntoUint8Array`（先给目标缓冲，按 `written` 截断）。**证据**：`devecocli build --modules hostruntime platform` 由"6 条告警"变成 **BUILD SUCCESSFUL 且这三类告警为 0**（包括那三条 `may throw exceptions` ——它们正是 accessSync 那三行）。

**又一处门禁盲区（顺手修掉）**：死代码门禁的"零消费者导出"正则**不认 `export async function`**（`async` 夹在中间）⇒ 所有异步导出的函数从来没被它看过。发现方式很偶然：删 `currentWindowId` 时才知道它一直躺在那儿（零调用点 + 实现里用了已弃用的 `getContext()`）。正则补上 `async` / `declare` / `abstract` 后重扫：774 个导出符号无死代码。

**门禁**：接线门禁新增**仓库级反面规则**（`dirs` 范围）：`accessSync` 在 `hostruntime/entry/appstate/platform/connection` 全域不得出现（唯一豁免 `FileProbe.ets` 自己）——**归真验证**：把任一哨兵改回 `fs.accessSync` ⇒ 当场红。真机判据新增 **D47**。

**同时定性（不修，写清理由）**：模块级权限告警（`connection` 的 6 条 `INTERNET`、`platform` 的 `KEEP_BACKGROUND_RUNNING`）是**库模块**编译时的告警——权限由 HAP 声明，`entry` 的 `module.json5` 已声明这三项并由 `tools/check-compliance.mjs` 与「应用内隐私说明」逐项对账；HAR 里再声明一次反而是错的。已写进 `docs/70`。

---

## P8-4b 手机上「模型」chip 被挤出屏幕——操作区静默消失

**缺陷**：输入区工具行是**一行 `Row` + `Blank()`**，里面七件固定宽度的控件（`@ 引用` / `/ 命令` / 附件 / 图片 / 计划 / 权限 / 模型）。`Row` 里不放 `layoutWeight` 的子元素**不会缩**，超出部分被直接裁掉 ⇒ 360vp 手机上**最右边的「模型」chip 被挤出屏幕**（它偏偏是手机上最常用的控件之一）。同一形态还在两处：**会话头视图切换行**（轨迹视图下右侧的「展开全部/收起全部/N 条事件」）、**时间总览统计行**（末尾的 TPS 那项）——都是"被裁掉时用户不会知道少了东西"（依据：`docs/08` P0 §7「操作区被内容挤压时不得静默消失」，该节逐字点名了 Composer 工具栏与会话 Header）。

**官方依据（逐字）**：官方 Web 输入区这一行的容器 CSS 就是 `flex-wrap: wrap; justify-content: space-between`
（`dsh-client-ui-conversation` 打包产物里的 composer 模块：`.W5Twba_row{flex-wrap:wrap;justify-content:space-between;align-items:center;gap:12px;…}`），
并把"模型"那一组放进 `.trailing{margin-left:auto}` ⇒ **放得下靠右、放不下换行，任何控件都不裁**。

**修法**：三处改成 `Flex({ wrap: FlexWrap.Wrap, justifyContent: FlexAlign.SpaceBetween })` + **两个组**
（左组工具/左组视图 chip、右组模型/右组动作），间距从 `space` 改成子元素 margin（换行后 `space` 只在主轴生效，跨行间距得靠 margin 才一致——照抄本仓 `NativeActionBar` 的既有写法）。顺带给消息元信息行里的**模型名**加上宽度上限与省略（次要信息，且切换入口在工具行）：不省略时它会把左边的时间/时长挤出屏幕。

**门禁**：接线门禁新增**第三种规则**「必含模式」（`REQUIRED_IN_FILE`）——`FlexWrap.Wrap` 这种**布局性质**在代码里就是一个符号，全局计数说不出"是哪个文件里的那一行"，于是钉成"某文件里必须出现某模式"，失败信息里写清"这一行为什么必须能换行"。**归真验证**：把工具行改回不换行的 `Row` ⇒ 门禁当场红在 `Composer.ets`。35 项接线 + 3 条必含模式全绿。真机判据新增 **D46**。

**如实说明**：布局行为**只能在真机上验收**（换行后的观感、大字号下的行高、软键盘弹起时的挤压顺序）——本机能证明的只是"这一行仍然是可换行的"与"编译通过"。D46 是这一条的验收入口。

---

## P8-4 改了一半的设置被"手一滑"关掉了——界面一句话都不说

**缺陷**：三个编辑浮层（凭据 / 文本设置 / 结构设置）都是"改一改就写进 Host"的编辑态，而浮层带拖拽条（`dragBar: true`）。用户改完一个数字或一段 JSON、手一滑把浮层拖没了 ⇒ `closeSheet()` 把草稿清干净，**界面上没有任何提示**。两条可预见的后果：① 他以为改动生效了（**假成功**——与 P7-13 那条"命令被拒却显示已执行"是同一类）；② 或者过一会儿发现值没变，再回来重改一遍，而原因无从得知。本仓的纪律是"界面不撒谎"：**放弃了什么，就要说出来**。

**依据**：`docs/08-UI与返回交互审计.md` 的 Sheet 一节（P1）：「关闭原因必须可区分：完成、取消、返回、系统关闭、外部状态变化」与「编辑态关闭前如果有未保存修改，必须有明确的取消/保存策略」。

**修法**：① 零依赖 `model/Sheets.ets` 增加 `sheetDiscardNotice(kind, label, draft, seed)` —— 判据是"草稿 ≠ 打开时的种子值"（凭据是只写不回显的，种子恒为空 ⇒ 非空即未保存）；枚举选择/目录选择/详情浮层**一律返回空串**（它们没有"未保存内容"可言，不能每次都弹一句废话）。② 宿主加**唯一的主动关闭执行点** `dismissSheetByUser()`：先算该说什么、取好回执该落到哪个域（`noteSettingWrite`），再 `closeSheet()`。③ 接在 `harmonySheetOptions` 的 `shouldDismiss` 上——那条路只有拖拽/遮罩/关闭按钮会走，**程序化关闭（保存成功后）不走它**，因此不会出现"保存成功了还说已放弃"的胡说。④ 打开浮层时记下种子值（`textSettingSeed` / `structSettingSeed`）。

**门禁**：fixture **993 → 1005 条**（12 条：文案四要素、没改过不吭声、凭据两种、三类无编辑态浮层）；**归真验证**：把 `sheetDiscardNotice` 改回"永远返回空串"（= 修前行为）⇒ **6 条红**。接线门禁新增「浮层关闭语义」项（34 → **35 项**）。真机判据新增 **D45**。

**仍缺（已登记）**：官方那种"关闭原因回传给调用方"的完整机制我们只做了**用户主动关闭**这一支；系统级关闭（`onDisappear` 由窗口变化/页面销毁触发）没有单独的语义，也不做"关闭前弹确认"（本仓在真机上看不到确认框就无法验收，宁可不加）。

---

## P8-3（后半）草稿"跨切换"做到了，"跨重载"还欠着——本轮把它补上

**缺口**：上一轮把草稿按会话存好了（切来切去不串台），但官方那句契约是两半：
`Composer draft (persisted; survives session switches **and reloads**)`。
我们只兑现了前半 ⇒ **应用一重启，所有没发出去的草稿全没了**。它被如实登记在矩阵 §6
（`hdsh-composer-drafts`）而不是含糊过去——这一轮就是去把那条登记划掉。

**修法**（三处，各司其职）：
① 纯模型 `ComposerDrafts` 增加**落盘编解码** `serializeDrafts` / `parseDrafts`：
   平行数组原样序列化（`Record` 会丢"最近写过"的顺序，而淘汰规则依赖它）；
   解析**宽容**——不是 JSON / 形状不对 / 元素类型不对 / **两数组长度不一致**一律当空或丢该项。
   长度一致性那条是刻意加的：错配会把甲的草稿显示在乙的会话里，正是上一轮刚修掉的那类错误。
② `platform/LocalPrefs` 增加 `composerDrafts` 一个**不透明字符串**键（读写各一个方法）。
   形状由模型决定、平台只搬字符串——`platform` 不许依赖 `appstate`，这条边界是编译器守着的。
③ 宿主：启动时读回**进袋子**（而不是直接填输入框——启动时还没有会话，"这段字属于谁"没有答案；
   用户点开哪个会话，草稿就在 `switchDraftContext` 里自然回来）；两个变更点
   （切会话 / 发送成功或被排队）各落一次盘。读回时**只在内存袋子还空**时采用磁盘值：
   `prefs.attach` 是异步的，这期间用户可能已经敲了字，覆盖它正是 P7-15 要避免的错。

**门禁**：fixture **983 → 993 条**（10 条专测编解码：往返、顺序、坏 JSON、形状不对、长度不一致、类型不对、空项、超上限裁剪）；**归真验证**：去掉"长度一致性"校验 ⇒ 1 条红。接线门禁把「每会话草稿」项扩到**八段**（模型编解码 + 平台读写 + 宿主两处调用点），任一段掉线都会红。真机判据新增 **D44**。

---

## P8-3 在会话 A 里写了一半，切到 B 顺手一按发送——那段话发给了 B

**缺陷**：草稿是 `Index` 上的**一个** `@State draft`（全应用一份），切会话时不动它。于是在会话 A 里写了一半、切到会话 B，输入框里还是 A 的那段话；用户顺手一按发送，**内容与收件人都错**，而他只会看到"怎么发到这儿了"。同一处还有两个连带问题：① 附件是**会话作用域**的（中枢切会话即清空），草稿却不是 ⇒ 同一个输入框里"文字属于上一个会话、附件属于这一个"；② 从会话退回列表再进来时文字仍在（看着像对的），但期间在别的会话里敲的内容会被冲掉。根因是一句话：**草稿被当成了"页面的临时状态"，而它其实是"会话视图状态的一部分"**。

**官方依据（逐字）**：`dsh-client-ui-conversation/lib/types/client/contract/views.d.ts` 第 19–20 行 ——
`/** Composer draft (persisted; survives session switches and reloads). */ draft: string;`

**修法**：新增零依赖 `appstate/model/ComposerDrafts.ets`——按会话存纯文本草稿（平行数组，`sessionIds[0]` 恒为"最近写过"，淘汰不需要时间戳；空文本 = 删条目；上限 24 个会话）；宿主在**唯一**的切换点 `switchDraftContext(nextSessionId)` 里走三步"存回当前会话 → 取出目标会话 → 填进输入框"，`openSession` 与"离开会话页签"两条路径都经过它；发送成功（或已进待发队列）后 `dropDraft` 丢掉那一份，避免切回来把已发出的原文又还给输入框。

**门禁**：fixture **967 → 983 条**（16 条专测这个模型）；**归真验证**：把 `draftOf` 改回"忽略会话 id"（= 修前行为）⇒ **3 条红**（"A 的草稿是 A 的"当场红）。接线门禁新增「每会话草稿」项（33 → **34 项**）。真机判据新增 **D43**。

**仍缺（已登记）**：官方那句契约的后半截 **"…and reloads"** —— 应用重启后草稿仍会丢（要落盘，`platform/LocalPrefs` 目前只存过数字/布尔/枚举，需要一个字符串键 + JSON 编解码 + 启动时序）。

---

## P8-6 一次网络抖动画出「一墙红色错误卡」（重试链被当成 N 次独立失败）

**缺陷**：`llm/retry` 的 `retryId` 被我（P7-18）读成了"每次重试唯一"，于是按 `轮次/步/第几次` 建条目 id ⇒ **同一次故障的每一次重试各占一条**，每条都是默认展开的红色错误卡。设备上网络一抖，用户看到的是"一墙错误"，而真相是**同一个请求在自动重试**——他既看不出这是同一次故障，也看不出它正在恢复。同轮还查出两个附件缺陷：① 链节点的状态此前用"或"合并（`next.started || previous.started`），一旦收到 `retry-started` 就**永远**停在"已重试"，后续等待的倒计时再也不出现；② 用户按了停止之后，那一行仍然写着「等待重试模型请求（3/5） · 7s」——**一个永远不会发生的承诺**。

**官方依据（逐字，两处）**：`dsh-llm-retry/lib/types/brand.d.ts` 的 `RetryId` = "**Stable identity shared by every attempt in one request-step retry chain.**"；生产者 `dsh-llm-retry/lib/index.js` 的 `recover()`：`const retryId = priorPolicyRetry?.data.retryId ?? RetryId(randomUUID())`（取同 turn/step/provider/policyKey 的上一条 `llm/retry` 的 id）⇒ **整条链一个 id**。官方界面据此把整条链渲染成**一个** `model-retry` 节点（`RetryState{turn, step, attempts[]}`，`dsh-client-ui-chat/.../conversation-nodes/retry.d.ts`）；状态与文案取自 `dsh-client-locale` 的 `message.retry.*`，渲染逻辑在 `ModelRetryItem`：`maximum = node.mode === 'normal' ? node.maxRetries : '∞'`、`retrySeconds(ms) = Math.max(1, Math.ceil(ms/1000))`、状态行模板 `{label}（{retry}/{maximum}） · {seconds}s`。

**修法**：① 链身份进 `dshcompat/EventShape`（新增 `RETRY_ID` 槽位，两个事件都带），条目 id 由纯函数 `Trajectory.retryItemId(attempt, chainId)` 算（无 `retryId` 的老 Host 退回"每次一行"，宁可多一行也不要把两次不同重试并成一条）；② 条目字段 `retryStarted: boolean` → `retryState: string`（`scheduled`/`started`/`cancelled`）——两个布尔会有"既没开始也没取消"这种不存在却可表示的组合；③ 合并规则：**序号更高就听新的**（回到"等待"），`started` 单向，`cancelled` 最强势；④ `cancelTurn()` 顺手把还在等待的重试标成"已取消"（`markPendingRetriesCancelled()`——这条状态**只能由客户端下结论**，上游没有"取消重试"的事件）；⑤ 文案换成官方**一行**模板 + 官方四个标签（含"模型请求重试已取消"）与官方秒数规则（最小 1 秒），视图补上官方 `重试延迟：` / `失败原因：` 两行；⑥ 删掉 `retryMode` 字段——它有写点没读点（官方也只是用它算 `maximum`，而那只取决于 `maxRetries` 有没有）。

**门禁**：fixture **947 → 967 条**；**归真验证两处**：把 id 规则改回"按次数" ⇒ 2 条红；把状态合并改回"或" ⇒ 1 条红（"下一次等待要回到等待"）。接线门禁的「重试行」项同步换成新符号（`retryStatusLine` / `retryItemId` / `RetryRow`）——**改名必须显式改它**，这正是它存在的意义。真机判据新增 **D42**。

**仍缺（已登记）**：官方节点保留整条 `attempts[]` 历史（我们只留最新一次的序号与原因，同一行能说清的就不重复列历史）；官方把它渲染成可展开的 `<details>`（我们恒展开——D3 §4 要求失败可见）。
