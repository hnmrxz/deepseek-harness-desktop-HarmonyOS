...
| 下一步（P2-6 / P4-7…） | ① 会话正文继续拆：`Content`（列表 + 尾部输入）/ `ProcessGroup` / `Answer` 三段（`ConversationPane` 仍有 1093 行，其中 build 里还混着列表与尾部）② 工具卡片的 applied 对照（需要投影补 `meta` 槽位）③ 右栏缺失项：面板滑入动画与拖拽调宽、单栏下切换器进 Sheet ④ 手机抽屉里的设置入口与四形态观感（真机，见 D17）⑤ **真机复验 E343 / D18** ——无真机不得宣称设备验收完成 |
