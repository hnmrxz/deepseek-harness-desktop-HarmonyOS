...
| 下一步（P4-6…） | ① 剩下的段都不大了：`pluginsTab` 已归 `SettingsPlugins`、核心段只差把 `CorePane` 的三段承载（提示 + 面板）也搬成 `SettingsCore`（现在 `SettingsPane` 里还内联着 30 行）② 设置项写入的统一事务与失败回执按域收口 ③ 手机抽屉里的设置入口与四形态观感（真机，见 D17） ④ **真机复验 E343**（冷启后点一下界面不再被杀进程）——无真机不得宣称设备验收完成 |
