...
| 下一步（P4-7…） | ① 设置域拆分已收口（九个域组件、宿主 500 行）——下一刀转向**主区/会话的信息架构**：`ConversationShell` 的 Header / Content / ProcessGroup / Answer 拆分，以及工具卡片的 applied 对照（需要投影补 `meta` 槽位）② 右栏缺失项：侧栏/右栏面板的滑入动画与拖拽调宽、单栏下切换器进 Sheet ③ 手机抽屉里的设置入口与四形态观感（真机，见 D17）④ **真机复验 E343**（冷启后点一下界面不再被杀进程）与 D18/D19 ——无真机不得宣称设备验收完成 |
