...
| 下一步（P4-5…） | ① 继续拆：`SettingsCore`、`SettingsPresets`、`SettingsSkills` —— `SettingsPane` 仍 **760 行**（`SettingsGeneral`/`SettingsDevice` 已于 P4-4 拆出），剩下的多是各段的呈现 ② 设置项写入的统一事务与失败回执按域收口 ③ 手机抽屉里的设置入口与四形态观感（真机，见 D17） ④ **真机复验 E343**（冷启后点一下界面不再被杀进程）——无真机不得宣称设备验收完成 |
